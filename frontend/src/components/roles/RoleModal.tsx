...
export function RoleEditModal({
  open, onClose, role, currentPermissionIds, onSubmit,
}: RoleEditModalProps) {
  if (!role) return null;

  return (
    <Modal open={open} onClose={onClose} title={`Cập nhật: ${role.role_name}`} size="xl">
      <RoleForm
        initialValues={{
...
export function RoleDeleteModal({
  open, onClose, role, onConfirm, loading = false,
    ...
    >
      <div className="flex flex-col items-center py-2 text-center">
        ...
        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
          Người dùng thuộc nhóm này sẽ mất quyền tương ứng.
        </p>
      </div>
    </Modal>
  );
