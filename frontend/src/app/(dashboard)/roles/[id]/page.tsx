...
export default function RoleDetailPage() {
  ...
  if (loading)
    return (
      <div>
        <Loading text="Đang tải thông tin nhóm quyền..." />
      </div>
    );
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <code className="rounded bg-slate-100 dark:bg-slate-700 px-2 py-0.5 text-sm text-slate-600 dark:text-slate-300">
              {role.role_code}
            </code>
          </div>
          {role.description && (
          ...
          <dl className="space-y-3">
            {[
              { label: "Tên nhóm", value: role.role_name },
              {
                label: "Mã",
                value: (
                  <code className="rounded bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 text-xs dark:text-slate-300">
                    {role.role_code}
                  </code>
                ),
              },
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-xl px-4 py-3 shadow-lg ${toast.type === "success" ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}
          role="alert"
        >
