...
def create_role(
    ...
    current_user = Depends(RequirePermission("role.create"))
):
...
def get_roles_full_details(
    ...
    limit: int = 100,
    db: Session = Depends(get_db),
    current_user = Depends(RequirePermission("role.view"))
):    
...
def update_role(
    ...
    db: Session = Depends(get_db),
    current_user = Depends(RequirePermission("role.update"))
):
...
def delete_role(
    role_id: int, 
    db: Session = Depends(get_db),
    current_user = Depends(RequirePermission("role.delete"))
):
