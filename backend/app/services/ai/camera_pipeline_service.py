...
class CameraProcessingPipelineService(
    ...
    def __init__(self, yunet_model_path: str = None, sface_model_path: str = None):
        ...
        print("VIDEO_PIPELINE_VERSION = camera_realtime_v18_snapshot_overlay")
    ...
    def _find_delayed_return_relink_candidate(
        ...
        *,
        ...
        for cand in ranked_candidates[:8]:
            ...
            forced_safe_chain = safe_hard or safe_stale_chain or safe_sibling
    ...
    def process_video(
        ...
        print("\n" + "=" * 60)
        print("KHỞI ĐỘNG ONLINE AI PIPELINE + NEW-FIRST CROSS-VIDEO SAFE RETURN V16")
        print("=" * 60)
        ...
        REASSIGN_IMMEDIATE_FACE = 0.992
        REASSIGN_IMMEDIATE_TOTAL = 0.972
        ...
        DELAYED_RETURN_MIN_TRACK_OBS = 35
        DELAYED_RETURN_MIN_SAMPLES = 4
        DELAYED_RETURN_HISTORY_MAX = 10
        DELAYED_RETURN_MIN_FACE_AVG = 0.968
        DELAYED_RETURN_MIN_TOTAL_AVG = 0.948
        DELAYED_RETURN_MIN_FACE_CONF_AVG = 0.80
        DELAYED_RETURN_MIN_APP_AVG = 0.68
        DELAYED_RETURN_MIN_BEST_MARGIN = 0.008
        DELAYED_RETURN_RISKY_MIN_FACE_AVG = 0.992
        DELAYED_RETURN_RISKY_MIN_TOTAL_AVG = 0.972
        DELAYED_RETURN_RISKY_MIN_BEST_MARGIN = 0.090
        ...
        with self.frame_extractor.create_temp_frame_dir() as frame_dir:
            ...
            for frame_data in frame_result.frames:
                        ...
                        if DELAYED_RETURN_RELINK_ENABLED:
                            ...
                            if eligible_return_track:
                                ...
                                if locked_pid:
                                    ...
                                    safe_candidates = []
                                    for c in (non_current_ranked or [])[:8]:
                                        ...
                                        if candidate_is_hub and not safe_hard_split_return and not safe_sibling_return and not safe_short_entry_return and not (face_v >= 0.995 and total_v >= 0.982 and app_v >= 0.90 and margin_v >= 0.060):
                                            print(
                                        ...
                                        if safe_non_risky or safe_risky or safe_hard_split_return or safe_stale_entry_chain or safe_sibling_return or safe_short_entry_return:
                                            c = dict(c)
                                    ...
                                    if safe_candidates:
                                        ...
                                        branch_label = 'hard_split' if bool(chosen_return.get('_safe_hard_split_return')) else ('stale_chain' if bool(chosen_return.get('_safe_stale_entry_chain')) else ('short_entry' if bool(chosen_return.get('_safe_short_entry_return')) else ('sibling' if bool(chosen_return.get('_safe_sibling_return')) else 'normal')))
                                        print(
                                        ...
                                        force_stale_chain = (
                                            ...
                                            and float(chosen_return.get('face', 0.0) or 0.0) >= 0.996
                                            and float(chosen_return.get('total', 0.0) or 0.0) >= 0.965
                                            and float(chosen_return.get('app', 0.0) or 0.0) >= 0.68
                                        )
                                        force_short_entry = (
                                        ...
                                        if force_hard_split or force_stale_chain or force_short_entry:
                                            if not self._is_profile_locked_by_other_track_in_frame(
                                                ...
                                                print(
                                                    f"[IDDBG_CONTROLLED_RETURN_FORCE_V15] track={track_id} "
                                                ...
                                                print(
                                                    f"[IDDBG_CONTROLLED_RETURN_FORCE_BLOCK_V15] track={track_id} "
                                ...
                                if delayed_candidate is not None:
                                    ...
                                    if new_profile_id and new_profile_id != old_profile_id:
                                        ...
                                        if moved:
                                            ...
                                            print(
                                                f"[IDDBG_CONTROLLED_RETURN_RELINK_V15] Track {track_id}: "
                                                f"{old_profile_id} -> {new_profile_id} | "
                                                f"samples={int(s.get('samples', 0))}, "
                                                f"avg_face={float(s.get('avg_face', 0.0)):.3f}, "
                                                f"avg_total={float(s.get('avg_total', 0.0)):.3f}, "
                ...
                debug_camera_records.extend(
                    self._snapshot_camera_debug_records(
                        frame_index=current_frame_index,
                ...
                if first_frame is not None:
                    ...
                    for frame_data in frame_result.frames:
                        ...
                        for record in frame_records:
                            ...
                            # Không kiểm tra exported_profile_ids vì camera thật không biết kết quả cuối.
...
camera_pipeline_service = CameraProcessingPipelineService(
    yunet_model_path=YUNET_MODEL_PATH,
